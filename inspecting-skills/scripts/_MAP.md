...
- **index_skill** (f) `(layout: SkillLayout)` :213
- **index_all_skills** (f) `(skills_root: Path)` :275
- **generate_registry** (f) `(skills_root: Path, output_path: Optional[Path] = None)` :289
...
- **register_skill** (f) `(skill_name: str, skill_path: Union[str, Path])` :188
- **SkillImportFinder** (C) :204
  - **find_spec** (m) `(self, fullname: str, path, target=None)` :212
  - **_create_spec** (m) `(self, fullname: str, skill_path: Path, parts: list[str])` :234
- **list_importable_skills** (f) `()` :282
