...
from .containerfile import ContainerLayer, parse_containerfile, content_hash
...
def cmd_build(args):
    ...
    if result.success:
        ...
        if result.snapshot_paths:
            print(f"  Snapshot paths: {', '.join(result.snapshot_paths)}")
        if result.env_vars:
...
def cmd_hash(args):
    ...
    h = content_hash(args.containerfile)
    print(h)
...
def cmd_inspect(args):
    ...
    print(f"Cache key: {content_hash(args.containerfile)}")
