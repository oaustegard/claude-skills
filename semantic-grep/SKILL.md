...
- `include` *(str)* — glob filter when `corpus` is a directory (default `"*.txt"`)
...
- **No persistent index.** Every call re-embeds the corpus. Fine for <~1K chunks; prohibitive for real knowledge bases. Phase 2: cache embeddings by content hash, optionally compress via remex.
- **Token budget is approximated by char count.** Gemini will reject overflow if the estimate is wrong; retry with shorter text.
- **Sequential batch groups.** `group_size=100` per HTTP call; groups run serially. For >1K chunks, add asyncio — not needed yet.
- **No CLI shim.** Called as a Python module, not a subprocess. Per design: "within an LLM rather than calling out to one." Adding argparse + entry_point is ~30 lines if ever needed.
